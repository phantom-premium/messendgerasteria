...
async function downloadMediaBatch(items) {
  ...
  // 2) Обычный браузер — тянем файлы и пробуем системный шэринг-лист.
  const files = await Promise.all(items.map(async (it, i) => {
    const resp = await fetch(it.url);
    const blob = await resp.blob();
    const filename = guessMediaFilename(it.url, it.type, items.length > 1 ? i : undefined);
    const mimeType = blob.type || (it.type === 'video' ? 'video/mp4' : 'image/jpeg');
  ...
  if (navigator.share && navigator.canShare && navigator.canShare({ files })) {
    try {
      ...
      // Пользователь сам закрыл системный лист — это не ошибка, просто
      // молча выходим, ничего дополнительно скачивать/показывать не надо.
      if (e && e.name === 'AbortError') return 'cancelled';
      // Любая другая ошибка шэринга (например, не поддержан именно этот
      // тип файла) — проваливаемся в blob-фолбэк ниже.
    }
  }

  // 3) Фолбэк для десктоп-браузеров без Web Share API.
  files.forEach((file) => {
...
async function downloadWholeLightboxAlbum() {
  ...
  try {
    ...
    else if (result === 'blob') showToast('Альбом сохранён');
  } catch (e) {
...
async function renderAdminApk() {
  ...
  el.innerHTML = `
    <div class="admin-apk-current">
      ${release ? `
        <div class="admin-apk-current-title">Сейчас раздаётся пользователям:</div>
        <div class="admin-apk-current-row">
          ...
          <a class="btn-secondary" href="/api/app/download" target="_blank" rel="noopener">Скачать</a>
        </div>
