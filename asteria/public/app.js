...
function renderMessageBubble(m) {
  ...
  else if (m.msgType === 'image') inner = `<img src="${m.mediaUrl}" data-lightbox="1">${m.content ? `<div class="msg-text">${formatMessageText(m.content)}</div>` : ''}`;
  ...
  bubble.innerHTML = senderName + replyBlock + inner + `<div class="time">${fmtTime(m.createdAt)}${editedTag}${readDot}</div>`;
