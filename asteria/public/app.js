...
function notifyNewMessage(message) {
  ...
  // Внутри приложения для Android обычный window.Notification (Web
  // Notifications API) не реализован штатным WebView — там его вообще нет,
  // и весь код ниже просто не выполнялся бы. Если страница открыта внутри
  // Asteria для Android, нативный код прокидывает мост window.AsteriaNotify
  // (см. WebAppInterface.java) — используем его, чтобы показать настоящее
  // системное уведомление.
  if (window.AsteriaNotify && typeof window.AsteriaNotify.show === 'function') {
    try { window.AsteriaNotify.show(title, body, message.conversationId); } catch (e) { /* игнорируем */ }
    return;
  }
