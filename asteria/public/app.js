...
function applyThemePref(pref) {
  ...
  updateNativeSystemBars();
}

// ---------- Верх/низ экрана в Android-приложении под цвет темы ----------
// Раньше статус-бар и панель навигации Android были зашиты в нативном коде
// одним фиксированным цветом — при светлой теме внутри чата низ экрана
// (панель навигации) оставался чёрным. Читаем РЕАЛЬНЫЙ отрисованный цвет
// конкретных панелей экрана (а не просто переменную темы --bg) — при
// открытом чате это шапка с именем собеседника (.chat-header) сверху и
// поле ввода (.composer) снизу, иначе — общий фон текущего раздела. Это
// надёжнее, чем полагаться на то, что переменная темы всегда совпадает с
// реальным цветом этих панелей (в этом самом проекте они уже не раз
// расходились и сходились обратно в ходе правок).
function resolveCssColorToRgb(cssColor) {
  const probe = document.createElement('div');
  probe.style.color = cssColor;
  probe.style.display = 'none';
  document.body.appendChild(probe);
  const computed = getComputedStyle(probe).color; // всегда придёт как "rgb(r, g, b)"
...
function isDarkColor([r, g, b]) {
  // Относительная яркость по стандартной формуле — темнее половины значит
  // фон тёмный, значит значкам статус-бара/навигации нужно быть светлыми,
  // чтобы не потеряться на нём (и наоборот).
  return (0.299 * r + 0.587 * g + 0.114 * b) / 255 < 0.5;
}
// Достаёт РЕАЛЬНЫЙ фон элемента, а не то, что написано в CSS-правиле —
// если фон прозрачный/не задан явно, поднимается вверх по родителям, пока
// не найдёт непрозрачный (иначе получим rgba(0,0,0,0), бесполезный сам по
// себе для решения "какой цвет красить снаружи страницы").
function resolveEffectiveBackground(el) {
  let node = el;
  while (node && node !== document.documentElement) {
    const bg = getComputedStyle(node).backgroundColor;
    const m = bg.match(/rgba?\(([\d.]+),\s*([\d.]+),\s*([\d.]+)(?:,\s*([\d.]+))?\)/);
    if (m && (m[4] === undefined || parseFloat(m[4]) > 0.5)) return [Number(m[1]), Number(m[2]), Number(m[3])];
    node = node.parentElement;
  }
  return resolveCssColorToRgb(getComputedStyle(document.documentElement).getPropertyValue('--bg').trim() || '#ffffff');
}
function updateNativeSystemBars() {
  if (!window.AsteriaNotify || typeof window.AsteriaNotify.setSystemBarColors !== 'function') return; // не Android-приложение — не при чём
  try {
    const chatOpen = $('#appScreen') && $('#appScreen').classList.contains('chat-open');
    const chatHeaderEl = chatOpen ? $('.chat-header') : null;
    const composerEl = chatOpen ? $('.composer') : null;
    const topRgb = chatHeaderEl ? resolveEffectiveBackground(chatHeaderEl) : resolveCssColorToRgb(getComputedStyle(document.documentElement).getPropertyValue('--bg').trim() || '#ffffff');
    const bottomRgb = composerEl ? resolveEffectiveBackground(composerEl) : resolveCssColorToRgb(getComputedStyle(document.documentElement).getPropertyValue('--bg').trim() || '#ffffff');
    window.AsteriaNotify.setSystemBarColors(rgbToHex(topRgb), rgbToHex(bottomRgb), isDarkColor(topRgb), isDarkColor(bottomRgb));
  } catch (e) { /* не критично — просто останутся цвета по умолчанию из themes.xml */ }
}
...
async function openProfilePage(userId) {
  ...
  const isOwnProfile = user.id === state.user.id;
  el.innerHTML = `
    <div class="profile-hero">
      <div class="my-avatar big xl${avatarRingClass(user)}" style="${avatarStyle(user)}">${user.avatar ? '' : initials(user.displayName)}</div>
      <div class="profile-hero-name">${escapeHtml(user.contactNickname || user.displayName)}${verifiedBadge(user)}${premiumBadge(user)}${user.isBot ? ' 🤖' : ''}</div>
      <div class="profile-hero-username">@${escapeHtml(user.username)}</div>
    </div>
    ${(!isOwnProfile && !user.isBot && !user.isContact) ? `<button class="btn-add-contact" id="addContactBtn">➕ Добавить в контакты</button>` : ''}
  ...
  // Кнопка "Изменить" в правом верхнем углу — только если человек уже в
  // контактах (иначе там просто показывать нечего, добавление — отдельной
  // синей кнопкой в теле профиля, см. выше).
  const editBtn = $('#editContactNicknameBtn');
  if (editBtn) {
    const showEdit = !isOwnProfile && !user.isBot && user.isContact;
    editBtn.classList.toggle('hidden', !showEdit);
    editBtn.onclick = showEdit ? () => openContactNicknameModal(user) : null;
  }
  ...
  if (addBtn) {
    addBtn.addEventListener('click', async () => {
      ...
      try {
        // Локальное имя сразу становится равным текущему видимому имени
        // человека (а не остаётся пустым) — так у контакта всегда есть
        // конкретное имя-отправная точка, а не "пусто, пока фолбэк на
        // displayName где-то в UI". При открытии "Изменить" видно именно
        // его, а не пустое поле.
        await api('/api/contacts', { method: 'POST', body: { userId: user.id, nickname: user.displayName } });
        await openProfilePage(user.id); // перерисовываем — теперь покажется "Изменить" вместо синей кнопки
      } catch (e) {
...
function openContactNicknameModal(user) {
  // Отправная точка для редактирования — уже заданное локальное имя, а
  // если его почему-то нет (старые записи и т.п.) — текущее видимое имя
  // человека, а не пустое поле без контекста.
  $('#contactNicknameInput').value = user.contactNickname || user.displayName || '';
  $('#contactNicknameInput').dataset.userId = user.id;
  showModal('#contactNicknameModal');
}
$('#saveContactNicknameBtn').addEventListener('click', async () => {
  const userId = $('#contactNicknameInput').dataset.userId;
  const nickname = $('#contactNicknameInput').value.trim();
  if (!userId) return;
  try {
    await api(`/api/contacts/${userId}`, { method: 'PATCH', body: { nickname } });
    $('#contactNicknameModal').classList.add('hidden');
    await openProfilePage(userId);
    await loadConversations(); // чтобы новое имя сразу подхватилось в списке чатов
  } catch (e) {
...
$('#removeContactBtn').addEventListener('click', async () => {
  const userId = $('#contactNicknameInput').dataset.userId;
  if (!userId) return;
  if (!confirm('Убрать этого человека из контактов? Переписка и история звонков останутся.')) return;
  try {
    await api(`/api/contacts/${userId}`, { method: 'DELETE' });
    $('#contactNicknameModal').classList.add('hidden');
    await openProfilePage(userId);
    await loadConversations();
  } catch (e) {
    alert(e.message || 'Не удалось убрать из контактов');
  }
});
...
const SETTINGS_SUBPAGES = {
  ...
  callDiag:      { title: 'Диагностика звонков',                                        el: 'subPageCallDiag' },
};
...
function openSettingsSubpage(key) {
  ...
  $('#settingsSubTitle').textContent = (cfg.titleKey && typeof t === 'function') ? t(cfg.titleKey) : cfg.title;
...
$('#runCallDiagBtn').addEventListener('click', runCallConnectivitySelfTest);

// ---------- Самопроверка сети для звонков (без второго собеседника) ----------
// ВАЖНО (это переписано специально, чтобы закрыть главный пробел старой
// версии): наличие "relay"-кандидата в ICE ещё ничего не доказывает —
// оно означает только то, что TURN Allocate дошёл до сервера по порту
// 3478. Реальный медиапоток идёт через ОТДЕЛЬНЫЙ диапазон релей-портов
// (см. TURN_RELAY_MIN_PORT/MAX_PORT на сервере, по умолчанию
// 49160-49260/UDP) — и именно этот диапазон чаще всего оказывается
// заблокирован снаружи (домашний роутер, облачный firewall провайдера),
// при этом Allocate на 3478 всё равно отвечает "ок". Раньше самопроверка
// на этом и останавливалась, показывая ложное "✅ TURN доступен" даже
// когда реальные звонки зависали без звука/видео.
//
// Теперь, если relay-кандидаты вообще нашлись, делается ВТОРОЙ, честный
// шаг: в этой же вкладке поднимаются два RTCPeerConnection с
// iceTransportPolicy:'relay' (запрещены host/srflx кандидаты — только
// релей), они соединяются друг с другом напрямую (SDP передаётся между
// ними локально, без сигналинг-сервера) и пытаются открыть DataChannel.
// Открыть его можно только если настоящие пакеты реально прошли через
// релей-порт в обе стороны — это и есть честная проверка ровно того
// сценария "звонок соединяется, а потом нет звука/видео".
async function runCallConnectivitySelfTest() {
  const btn = $('#runCallDiagBtn');
  const resultEl = $('#callDiagResult');
  btn.disabled = true;
  btn.textContent = '⏳ Проверяю…';
  resultEl.innerHTML = '<div style="color:var(--text-secondary); font-size:13px;">Шаг 1/2: собираю ICE-кандидаты…</div>';

  const found = { host: 0, srflx: 0, relay_udp: 0, relay_tcp: 0 };
  let pc = null;
  try {
    await ensureRtcConfig();
    pc = new RTCPeerConnection(RTC_CONFIG);
    pc.createDataChannel('diag'); // нужен хоть один канал/трек, иначе ICE-сбор не запустится
    pc.onicecandidate = (e) => {
      if (!e.candidate) return;
      const c = e.candidate.candidate || '';
      const typeMatch = c.match(/typ (\w+)/);
      const protoMatch = c.match(/^candidate:\S+ \d+ (\w+)/);
      const candType = typeMatch ? typeMatch[1] : null;
      const proto = protoMatch ? protoMatch[1].toLowerCase() : null;
      if (candType === 'relay') found[proto === 'tcp' ? 'relay_tcp' : 'relay_udp']++;
      else if (candType === 'host') found.host++;
      else if (candType === 'srflx') found.srflx++;
    };
    const offer = await pc.createOffer();
    await pc.setLocalDescription(offer);
    // Ждём завершения сбора (или тайм-аут — на некоторых сетях сбор relay-
    // кандидатов через TURN занимает пару секунд).
    ...
    resultEl.innerHTML = `<div class="settings-error">Не удалось выполнить проверку: ${escapeHtml(e.message || String(e))}</div>`;
    btn.disabled = false;
    btn.textContent = '🩺 Проверить соединение сейчас';
    if (pc) pc.close();
    return;
  }
  if (pc) pc.close();

  // Шаг 2: честная проверка релея, только если вообще есть смысл её делать.
  let relayReal = { attempted: false, ok: false, error: null };
  if (found.relay_udp > 0 || found.relay_tcp > 0) {
    resultEl.innerHTML = '<div style="color:var(--text-secondary); font-size:13px;">Шаг 2/2: проверяю, реально ли данные проходят через релей-порт (не только Allocate)…</div>';
    relayReal.attempted = true;
    try {
      relayReal.ok = await testRealRelayDataFlow();
    } catch (e) {
      relayReal.ok = false;
      relayReal.error = (e && e.message) ? e.message : String(e);
    }
  }
  ...
  const row = (ok, label, hint) => `
    <div style="display:flex; align-items:flex-start; gap:8px; padding:8px 0; border-bottom:1px solid var(--border);">
      ...
      <div>
        ...
        <div style="font-size:12.5px; color:var(--text-secondary);">${escapeHtml(hint)}</div>
      </div>
    </div>`;

  let html = '';
  html += row(found.host > 0, 'Прямые кандидаты (host)', found.host > 0 ? `Найдено: ${found.host}. Пригодится только если собеседник в этой же локальной сети.` : 'Не найдено — необычно, обычно есть всегда.');
  html += row(found.srflx > 0, 'STUN (srflx)', found.srflx > 0 ? `Найдено: ${found.srflx}. Виден внешний адрес — часть NAT пройдёт напрямую.` : 'Не найдено — STUN-сервер недоступен с этого устройства.');
  html += row(found.relay_udp > 0, 'TURN через UDP (Allocate)', found.relay_udp > 0 ? `Найдено: ${found.relay_udp}. Порт 3478/UDP отвечает — но это ещё не проверка релея медиапотока, см. ниже.` : 'Не найдено — либо TURN-сервер не запущен, либо порт 3478/UDP не проброшен наружу, либо эта сеть блокирует исходящий UDP.');
  html += row(found.relay_tcp > 0, 'TURN через TCP (Allocate)', found.relay_tcp > 0 ? `Найдено: ${found.relay_tcp}. Порт 3478/TCP отвечает — резервный путь, если сеть блокирует UDP.` : 'Не найдено — проверьте порт 3478/TCP на сервере.');

  if (relayReal.attempted) {
    html += row(
      relayReal.ok,
      'Реальный медиапоток через релей-порт',
      relayReal.ok
        ? 'Настоящие данные прошли через релей-порт в обе стороны — сам TURN-релей работает по-честному.'
        : `Не прошли (${relayReal.error === 'timeout' ? 'таймаут ~10 сек' : escapeHtml(relayReal.error || 'ошибка ICE')}). Allocate на 3478 отвечает, но диапазон релей-портов (по умолчанию 49160-49260/UDP) снаружи не достучаться — почти всегда это домашний роутер или облачный firewall провайдера (AWS Security Group / DO-Timeweb Cloud Firewall и т.п.), которые не пропускают именно этот диапазон.`
    );
  }

  const hasAnyRelay = found.relay_udp > 0 || found.relay_tcp > 0;
  let verdict;
  if (!hasAnyRelay) {
    verdict = '<div style="margin-top:10px; padding:10px; border-radius:10px; background:rgba(239,68,68,.12); color:#dc2626; font-weight:600;">⚠️ TURN-сервер недоступен ни по UDP, ни по TCP с этого устройства — звонки с собеседником в другой сети, скорее всего, не установятся. Проверьте порт 3478 на сервере (и UDP, и TCP) и логи сервера при старте.</div>';
  } else if (relayReal.attempted && !relayReal.ok) {
    verdict = '<div style="margin-top:10px; padding:10px; border-radius:10px; background:rgba(239,68,68,.12); color:#dc2626; font-weight:600;">⚠️ TURN отвечает на служебные запросы, но реальный медиапоток через релей-порт не проходит — это ровно причина "звонок соединяется, а звука/видео нет". Откройте диапазон релей-портов (49160-49260/UDP по умолчанию) снаружи: в файрволе ОС (сервер обычно делает это сам при root) И отдельно в панели роутера/облачного хостинга (это сервер сам сделать не может).</div>';
  } else {
    verdict = '<div style="margin-top:10px; padding:10px; border-radius:10px; background:rgba(34,197,94,.12); color:#16a34a; font-weight:600;">✅ TURN-релей проверен по-честному и реально работает с этого устройства — если звонок с конкретным собеседником всё равно не проходит, дело, скорее всего, не в TURN-сервере, а в сети именно этого собеседника (проверьте диагностику того же звонка со стороны обоих участников).</div>';
  }

  resultEl.innerHTML = html + verdict;
  btn.disabled = false;
  btn.textContent = '🩺 Проверить соединение сейчас';
}

// Честная проверка релея "из конца в конец": два RTCPeerConnection в этой
// же вкладке, оба принудительно только на relay-кандидатах (host/srflx
// запрещены политикой iceTransportPolicy:'relay'), соединяются друг с
// другом напрямую — SDP/кандидаты передаются между ними в памяти браузера,
// сигналинг-сервер не нужен. Если DataChannel открылся и по нему реально
// прошло сообщение — значит настоящие пакеты дошли через выделенный
// релей-порт в обе стороны. Если нет — либо ICE зафейлился (relay-порт
// недостижим снаружи), либо истёк таймаут.
function testRealRelayDataFlow(timeoutMs = 10000) {
  return new Promise((resolve, reject) => {
    const cfg = { iceServers: RTC_CONFIG.iceServers, iceTransportPolicy: 'relay' };
    let pc1, pc2;
    let settled = false;
    const finish = (ok, err) => {
      if (settled) return;
      ...
      if (ok) resolve(true); else reject(err || new Error('unknown'));
    };
    ...
    try {
      ...
      pc1.oniceconnectionstatechange = () => {
        if (pc1.iceConnectionState === 'failed') finish(false, new Error('ice-failed'));
      };
      pc2.oniceconnectionstatechange = () => {
        if (pc2.iceConnectionState === 'failed') finish(false, new Error('ice-failed'));
      };
      const dc1 = pc1.createDataChannel('relay-probe');
      dc1.onopen = () => { try { dc1.send('ping'); } catch (e) {} };
      pc2.ondatachannel = (e) => {
        e.channel.onmessage = (ev) => { if (ev.data === 'ping') finish(true); };
      };
      pc1.createOffer()
        .then((offer) => pc1.setLocalDescription(offer).then(() => offer))
        .then((offer) => pc2.setRemoteDescription(offer))
        .then(() => pc2.createAnswer())
        .then((answer) => pc2.setLocalDescription(answer).then(() => answer))
...
// Человекочитаемые подписи для типов ICE-кандидатов/пар — используются и
// тут, и в результатах самопроверки runCallConnectivitySelfTest выше.
const ICE_TYPE_LABELS = { host: 'напрямую (host)', srflx: 'через STUN', relay: 'через TURN (relay)' };
const EVENT_KIND_LABELS = {
  'ice-gathering-complete': 'Сбор ICE-кандидатов завершён',
  'iceConnectionState': 'Состояние ICE-соединения',
  'connectionState': 'Состояние соединения',
  'ice-restart-triggered': 'Запущен перезапуск ICE (restartIce)',
  'gave-up-after-8s-failed': 'Соединение не восстановилось за 8с — звонок завершён',
  'media-watchdog-stuck': 'Входящее медиа не пошло — заподозрен обрыв',
};
function renderCallDiagSide(role, label, d) {
  if (!d) return `<div class="modal-subtitle">${escapeHtml(label)}</div><div class="chat-header-sub" style="padding:0 2px 10px;">Диагностика с этой стороны не была записана (например, старая версия приложения, либо звонок не дошёл до этой стороны вовсе).</div>`;
  const g = d.candidatesGathered || {};
  const pair = d.selectedPair;
  const pairText = pair
    ? `${ICE_TYPE_LABELS[pair.localType] || pair.localType || '?'} (${pair.localProtocol || '?'}) ↔ ${ICE_TYPE_LABELS[pair.remoteType] || pair.remoteType || '?'} (${pair.remoteProtocol || '?'})`
    : 'не установлена';
  const timeline = (d.events || []).map((ev) => {
    const label2 = EVENT_KIND_LABELS[ev.kind] || ev.kind;
    const extra = ev.state ? `: ${ev.state}` : '';
    return `<div style="font-size:12px; color:var(--text-secondary); padding:2px 0;">+${(ev.t / 1000).toFixed(1)}с — ${escapeHtml(label2)}${escapeHtml(extra)}</div>`;
  }).join('');
  return `
    <div class="modal-subtitle">${escapeHtml(label)}</div>
    <div style="font-size:13px; line-height:1.7;">
      <div>Выбранный путь соединения: <b>${escapeHtml(pairText)}</b></div>
      <div>Собрано кандидатов — host: ${g.host || 0}, srflx: ${g.srflx || 0}, relay-UDP: ${g.relay_udp || 0}, relay-TCP: ${g.relay_tcp || 0}</div>
      <div>Финальное состояние ICE: <b>${escapeHtml(d.finalIceState || '—')}</b>, соединения: <b>${escapeHtml(d.finalConnectionState || '—')}</b></div>
      ${d.mediaStuck ? '<div style="color:#dc2626; font-weight:600;">⚠️ Зафиксирован обрыв входящего медиа во время звонка</div>' : ''}
      ${d.network ? `<div>Сеть на момент звонка: ${escapeHtml(d.network.effectiveType || d.network.type || '—')}</div>` : ''}
      <div style="color:var(--text-secondary); word-break:break-all; font-size:11.5px; margin-top:4px;">${escapeHtml(d.userAgent || '')}</div>
    </div>
    ${timeline ? `<div style="margin-top:8px;">${timeline}</div>` : ''}
  `;
}
function showCallDiagnostics(call) {
  const diag = call.diagnostics || {};
  const body = $('#callDiagModalBody');
  body.innerHTML = renderCallDiagSide('caller', 'Звонивший', diag.caller) + '<hr style="border-color:var(--border); margin:14px 0;">' + renderCallDiagSide('callee', 'Принимавший', diag.callee);
  showModal('#callDiagModal');
}
...
function renderCallHistory(calls) {
  ...
  calls.forEach((c) => {
    ...
    if (c.mode === '1:1') {
      ...
      row.innerHTML = `
        ...
        ${c.diagnostics ? `<button class="icon-btn" data-diag="${c.id}" title="Диагностика звонка">🩺</button>` : ''}
  ...
  el.querySelectorAll('[data-diag]').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      const call = calls.find((c) => c.id === btn.dataset.diag);
      if (call) showCallDiagnostics(call);
    });
  });
}
...
// Структурированная (не просто текстовые строки в консоль, как callDiag)
// сводка диагностики текущего звонка — собирается по ходу звонка в
// attachPeerConnectionHandlers, отправляется на сервер и сохраняется вместе
// с записью о звонке при его завершении (см. sendCallDiagnostics,
// вызывается из cleanupCall). Это то, что потом можно посмотреть в истории
// звонков, а не только "если успел посмотреть в терминал сервера вовремя".
let callDiagSummary = null;
function createCallDiagSummary(callId, kind, direction) {
  return {
    callId,
    kind,
    ...
    network: (navigator.connection && (navigator.connection.effectiveType || navigator.connection.type))
      ? { effectiveType: navigator.connection.effectiveType, type: navigator.connection.type }
      : null,
    candidatesGathered: { host: 0, srflx: 0, relay_udp: 0, relay_tcp: 0, other: 0 },
    events: [], // {t: мс от начала, ...}
    selectedPair: null, // выясняется по факту через getStats при первом connected/completed
    mediaStuck: false,
    finalIceState: null,
    finalConnectionState: null,
  };
}
function callDiagEvent(kind, extra) {
  if (!callDiagSummary) return;
  callDiagSummary.events.push({ t: Date.now() - callDiagSummary.startedAt, kind, ...extra });
}
function sendCallDiagnostics() {
  if (!callDiagSummary || !callDiagSummary.callId) { callDiagSummary = null; return; }
  const role = callDiagSummary.direction === 'outgoing' ? 'caller' : 'callee';
  const summary = callDiagSummary;
  callDiagSummary = null;
  // Best-effort: диагностика не должна мешать обычной работе приложения,
  // если отправка не удалась (звонок уже совсем оборвался, сети нет и
  // т.п.) — просто молча пропускаем.
  api(`/api/calls/${encodeURIComponent(summary.callId)}/diagnostics`, { method: 'POST', body: { role, summary } }).catch(() => {});
}
...
function attachPeerConnectionHandlers(pc) {
  ...
  pc.onicecandidate = (e) => {
    if (e.candidate) {
      ...
      const typeMatch = c.match(/typ (\w+)/);
      const protoMatch = c.match(/^candidate:\S+ \d+ (\w+)/);
      const candType = typeMatch ? typeMatch[1] : '?';
      const proto = protoMatch ? protoMatch[1].toLowerCase() : '?';
      diag('ICE-кандидат собран:', candType, 'proto=', proto);
      if (callDiagSummary) {
        // relay различаем по протоколу отдельно (UDP/TCP) — это и есть
        // ключевая метрика для вопроса "звонки работают только в одном
        // Wi-Fi": если relay_tcp почти всегда есть, а relay_udp иногда
        // пуст — где-то по пути блокируется исходящий UDP.
        const key = candType === 'relay' ? (proto === 'tcp' ? 'relay_tcp' : 'relay_udp')
          : (candType === 'host' || candType === 'srflx') ? candType : 'other';
        callDiagSummary.candidatesGathered[key] = (callDiagSummary.candidatesGathered[key] || 0) + 1;
      }
      ...
      callDiagEvent('ice-gathering-complete');
    }
  };
  pc.ontrack = (e) => {
  ...
  pc.oniceconnectionstatechange = () => {
    ...
    callDiagEvent('iceConnectionState', { state: st });
    if (callDiagSummary) callDiagSummary.finalIceState = st;
    if (st === 'connected' || st === 'completed') {
      ...
      pc.getStats().then((stats) => {
        stats.forEach((r) => {
          if (r.type === 'candidate-pair' && r.state === 'succeeded') {
            ...
            if (callDiagSummary) {
              callDiagSummary.selectedPair = {
                localType: local && local.candidateType, localProtocol: local && local.protocol,
                remoteType: remote && remote.candidateType, remoteProtocol: remote && remote.protocol,
              };
            }
          }
        });
      ...
      sawRealFailure = true;
      callDiagEvent('ice-restart-triggered');
      if (typeof pc.restartIce === 'function') { try { pc.restartIce(); } catch (e) {} }
      if (!reconnectTimer) {
        ...
        reconnectTimer = setTimeout(() => {
          ...
          if (pc.iceConnectionState === 'failed') {
            callDiagEvent('gave-up-after-8s-failed');
  ...
  pc.onconnectionstatechange = () => {
    ...
    callDiagEvent('connectionState', { state: pc.connectionState });
    if (callDiagSummary) callDiagSummary.finalConnectionState = pc.connectionState;
  };
  ...
  state.callMediaWatchdogInt = setInterval(async () => {
    ...
    if (!watchdogSawMedia && watchdogTicks >= 3) {
      if (!state.callMediaStuck) {
        ...
        if (callDiagSummary) callDiagSummary.mediaStuck = true;
        callDiagEvent('media-watchdog-stuck', { iceState: pc.iceConnectionState, connState: pc.connectionState });
...
async function startCall(kind) {
  ...
  callDiagSummary = createCallDiagSummary(state.currentCallId, kind, 'outgoing');
...
async function acceptIncomingCall() {
  ...
  callDiagSummary = createCallDiagSummary(pendingOffer.callId, pendingOffer.kind, 'incoming');
  state.callConnected = false;
  pendingCallIce = [];
  state.currentFacingMode = 'user';
  state.localStream = await acquireCallMedia();
...
function cleanupCall() {
  ...
  stopRingback();
  stopRingtone();
  // Диагностика — отправляем ДО закрытия pc (сама sendCallDiagnostics не
  // трогает pc, только уже накопленный объект callDiagSummary, но порядок
  // важен по смыслу: "итог звонка" логичнее фиксировать до его закрытия).
  sendCallDiagnostics();
...
$all('.modal-tab[data-atab]').forEach((tab) => {
  tab.addEventListener('click', () => {
    ...
    $('#adminExportTab').classList.toggle('hidden', t !== 'export');
  });
});

async function openAdminPanel() {
  await Promise.all([renderAdminUsers(), renderAdminChats(), renderAdminApk(), renderAdminBanner(), renderAdminExport()]);
}

function renderAdminExport() {
  const el = $('#adminExportTab');
  el.innerHTML = `
    <div class="chat-header-sub" style="padding: 4px 2px 14px;">
      Полная выгрузка всех данных сервера — пользователи, все чаты и сообщения, звонки, контакты и т.д. Доступно два формата.
    </div>
    <div class="settings-list-group" style="padding:14px; display:flex; flex-direction:column; gap:6px;">
      <div style="font-weight:600;">🗜️ Полный архив (.zip) — для переезда на другой сервер</div>
      <div class="chat-header-sub" style="padding:0;">База данных + все загруженные файлы (аватарки, обои, сторис) одним архивом. На новом сервере достаточно распаковать этот zip прямо в домашнюю папку пользователя, от которого запускается сервер (<code>unzip asteria-full-backup-....zip -d ~</code>) — раскладывать файлы по подпапкам вручную не нужно.</div>
      <a class="btn-primary" id="exportArchiveBtn" href="/api/admin/export/archive" style="text-align:center; text-decoration:none; margin-top:6px;">Скачать архив .zip</a>
    </div>
    <div class="settings-list-group" style="padding:14px; display:flex; flex-direction:column; gap:6px; margin-top:12px;">
      <div style="font-weight:600;">📦 Только файл базы данных (.db)</div>
      <div class="chat-header-sub" style="padding:0;">Без загруженных файлов — только сами записи (пользователи, чаты, сообщения и т.д.). Пригодится, если файлы переносите отдельно.</div>
      <a class="btn-secondary" id="exportDbBtn" href="/api/admin/export/db" style="text-align:center; text-decoration:none; margin-top:6px;">Скачать .db</a>
    </div>
    <div class="settings-list-group" style="padding:14px; display:flex; flex-direction:column; gap:6px; margin-top:12px;">
      <div style="font-weight:600;">📄 JSON</div>
      <div class="chat-header-sub" style="padding:0;">Человекочитаемый формат — удобно посмотреть данные глазами или перенести куда-то ещё, помимо SQLite.</div>
      <a class="btn-secondary" id="exportJsonBtn" href="/api/admin/export/json" style="text-align:center; text-decoration:none; margin-top:6px;">Скачать .json</a>
    </div>
    <div class="settings-footnote" style="padding:14px 2px 0;">⚠️ Выгрузка содержит все данные без исключений, включая служебные (сессии, push-подписки). Храните файл так же аккуратно, как сам доступ к серверу.</div>
  `;
}
