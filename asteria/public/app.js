...
function renderMessageBubble(m) {
  ...
  else if (m.msgType === 'image') inner = `<img src="${m.mediaUrl}" data-lightbox="1">`;
  ...
  const hasCaption = m.msgType === 'image' && !!m.content;
  const isBareMedia = m.msgType === 'video' || m.msgType === 'video_circle' || (m.msgType === 'image' && !m.content);
  let timeBlock = '';
  if (hasCaption) {
  ...
  if (m.msgType === 'text') {
    ...
    copyBtn.addEventListener('click', () => copyMessageText(m));
...
function setupCircleVideoPlayer(bubble) {
  ...
  const expand = () => {
    ...
    bubble._circleAnchor = { cx: rect.left + rect.width / 2, cy: rect.top + rect.height / 2 };
    // Заглушка на месте пузыря в потоке сообщений, чтобы список не
    // «прыгал», пока сам пузырь уже position:fixed и растёт поверх интерфейса.
