...
function buildConvItem(conv) {
  ...
  const nameBadge = conv.type === 'dm' ? verifiedBadge(conv.peer) + premiumBadge(conv.peer) : '';
  const typeIcon = conv.type === 'channel' ? ' 📢' : conv.type === 'group' ? ' 👥' : '';
  const unreadHtml = conv.unreadCount ? `<span class="conv-badge">${conv.unreadCount > 99 ? '99+' : conv.unreadCount}</span>` : '';
  meta.innerHTML = `<div class="conv-name"><span>${pinned ? '📌 ' : ''}${escapeHtml(title)}${nameBadge}${typeIcon}</span>${unreadHtml}</div><div class="conv-last">${escapeHtml(preview)}</div>`;
...
async function runChatsSearch() {
  ...
  if (botMatches.length) {
    ...
    botMatches.forEach((b) => {
      const row = document.createElement('div');
      row.className = 'channel-row';
      row.innerHTML = `<div class="avatar" style="${b.avatar ? avatarStyle(b) : ''}">${b.avatar ? '' : initials(b.displayName)}</div><div>${escapeHtml(b.displayName)} 🤖<div class="chat-header-sub">@${escapeHtml(b.username)}</div></div>`;
      row.addEventListener('click', async () => {
...
function renderChatHeader(conv) {
  ...
  const badge = conv.type === 'dm' ? verifiedBadge(conv.peer) + premiumBadge(conv.peer) : '';
...
function renderMessageBubble(m) {
  ...
  else if (m.msgType === 'video') inner = `<div class="media-frame media-frame-video"><video class="media-frame-bg-video" src="${m.mediaUrl}" muted autoplay loop playsinline></video><video class="media-frame-video-fg" src="${m.mediaUrl}" controls playsinline></video></div>`;
  ...
  // Фото/видео/кружок показываем как в Telegram: медиа во весь размер
  ...
  const hasCaption = (m.msgType === 'image' || m.msgType === 'video') && !!m.content;
  const isBareMedia = m.msgType === 'video_circle' || ((m.msgType === 'video' || m.msgType === 'image') && !m.content);
  let timeBlock = '';
  if (hasCaption) {
  ...
  bubble.innerHTML = senderName + replyBlock + inner + timeBlock;

  // Баг «подпись наезжает на картинку, пока телефон не повернуть»:
  // .bubble.media-only.has-caption — grid-контейнер, а сама картинка/видео
  // внутри квадратной рамки (.media-frame) держит форму через
  // aspect-ratio:1/1. При ПЕРВОЙ отрисовке браузер (особенно мобильный
  // WebKit/Chrome) иногда успевает посчитать высоту grid-строк ДО того,
  // как узнал реальные размеры самой картинки/видео — получается замкнутая
  // зависимость (aspect-ratio нужна ширина рамки ⇄ ширина рамки — из
  // grid-колонки ⇄ высота grid-строки — из содержимого), и полоса подписи
  // "уезжает" не под картинку, а поверх/под неё внахлёст. Поворот экрана
  // чинит это только потому, что сам вызывает полный пересчёт раскладки
  // (resize) — а нам нужно вызвать точно такой же пересчёт самим, сразу
  // как только у картинки/видео появились настоящие размеры, а не ждать,
  // пока пользователь случайно повернёт телефон.
  if (bubble.classList.contains('has-caption')) {
    const mediaEl = bubble.querySelector('.media-frame-img, .media-frame-video-fg');
    if (mediaEl) {
      const forceReflow = () => {
        ...
        // закешированные (ещё «до-загрузочные») размеры grid-строк внутри.
        // Оба присваивания синхронные и происходят в одном тике до отрисовки
      ...
      if (mediaEl.tagName === 'IMG') {
        if (mediaEl.complete) forceReflow();
        else mediaEl.addEventListener('load', forceReflow, { once: true });
      } else {
        if (mediaEl.readyState >= 1) forceReflow();
        else mediaEl.addEventListener('loadedmetadata', forceReflow, { once: true });
      }
    }
  }
  ...
  if (m.msgType === 'video_circle') setupCircleVideoPlayer(bubble);

  // кнопка расшифровки голосового сообщения в текст
  if (m.msgType === 'voice') {
    const voiceWrap = bubble.querySelector('.voice-msg-wrap');
    if (voiceWrap) {
      const audioEl = voiceWrap.querySelector('.voice-audio');
      const voiceToTextBtn = voiceWrap.querySelector('.voice-totext-btn');
      voiceToTextBtn.addEventListener('click', transcribeBtnClickHandler(voiceToTextBtn, audioEl.getAttribute('src'), bubble, '.voice-msg-wrap'));
    }
  }
...
/* ---------------- РАСШИФРОВКА ГОЛОСА В ТЕКСТ ----------------
   Раньше это было сделано через браузерный Web Speech API (слушал
   микрофон, пока играла запись) — работало нестабильно и требовало
   разрешения на микрофон. Теперь расшифровка идёт через внешний
   бесплатный сервис — Hugging Face Inference API (модель Whisper).
   Сам HTTP-запрос к внешнему API и токен доступа живут на сервере
   (см. /api/transcribe в server.js) — клиент лишь передаёт mediaUrl
   уже загруженного файла и получает готовый текст. */

// Выводит результат расшифровки прямо под медиа внутри пузыря сообщения.
...
function transcribeBtnClickHandler(btn, mediaUrl, bubble, anchorSelector) {
  return async (e) => {
    e.stopPropagation();
    ...
    btn.textContent = '…';
    try {
      const { text } = await api('/api/transcribe', { method: 'POST', body: { mediaUrl } });
      showTranscriptResult(bubble, anchorSelector, text);
    } catch (err) {
      showToast((err && err.message) || 'Не удалось распознать речь');
    } finally {
...
function setupCircleVideoPlayer(bubble) {
  ...
  toTextBtn.addEventListener('click', transcribeBtnClickHandler(toTextBtn, video.getAttribute('src'), bubble, '.circle-video-wrap'));
}
...
function replyPreviewLabel(m) {
  ...
  const map = { image: '🖼 Фото', video: '🎬 Видео', video_circle: '🎥 Видеосообщение', voice: '🎙 Голосовое сообщение', music: '🎵 Трек', file: '📄 Файл', sticker: '😊 Стикер', poll: '📊 ' + (m.content || 'Опрос') };
  return map[m.msgType] || m.content || '';
}
...
// Прокручивает к оригинальному сообщению и на секунду подсвечивает его —
// используется по клику на цитату внутри пузыря с ответом.
...
const SETTINGS_SUBPAGES = {
  premium:       { title: 'Asteria Premium',       el: 'subPagePremium' },
...
// верх списка) — разворачиваются обратно. Небольшой гистерезис (COLLAPSE_AT /
// EXPAND_AT) не даёт анимации дёргаться туда-сюда у порога.
