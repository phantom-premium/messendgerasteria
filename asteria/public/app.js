...
// Эксклюзивные готовые фоны чата (градиенты, без загрузки файла)
const PREMIUM_WALLPAPERS = [
  { id: 'aurora', label: 'Аврора', css: 'linear-gradient(135deg,#3a1c71,#d76d77,#ffaf7b)' },
  { id: 'nebula', label: 'Небула', css: 'linear-gradient(135deg,#0f2027,#203a43,#2c5364)' },
  { id: 'gold', label: 'Золото', css: 'linear-gradient(135deg,#bf953f,#fcf6ba,#b38728,#fbf5b7,#aa771c)' },
];
...
// Класс для анимированного золотого кольца вокруг аватара Premium-пользователя.
// Подставляется в атрибут class у элемента .avatar.
function avatarRingClass(u) {
  return u && isPremiumActive(u) ? ' avatar-premium-ring' : '';
}
...
function toggleAvatarRing(el, u) {
  if (!el) return;
  el.classList.toggle('avatar-premium-ring', isPremiumActive(u));
}
...
// Готовая галерея премиум-фонов чата — градиенты без загрузки файла.
// Обычным пользователям показываются с 🔒, клик объясняет про подписку.
function renderWallpaperPresets() {
  const el = $('#wallpaperPresets');
  if (!el) return;
  const premium = isPremiumActive(state.user);
  el.innerHTML = '';
  PREMIUM_WALLPAPERS.forEach((wp) => {
    const btn = document.createElement('button');
    btn.className = 'wallpaper-preset' + (premium ? '' : ' locked');
    btn.style.backgroundImage = wp.css;
    btn.title = premium ? wp.label : `${wp.label} — доступно с Asteria Premium`;
    btn.innerHTML = premium ? `<span class="wallpaper-preset-label">${escapeHtml(wp.label)}</span>` : `<span class="wallpaper-preset-lock">⭐</span>`;
    btn.addEventListener('click', async () => {
      if (!premium) { alert(`Фон «${wp.label}» — привилегия Asteria Premium (${PREMIUM_PRICE_LABEL}). Подписку выдаёт администратор.`); return; }
      applyWallpaperPref(wp.css);
      try {
        const { user } = await api('/api/me', { method: 'PATCH', body: { chatWallpaper: wp.css } });
        state.user = user;
      } catch (e) { alert('Не удалось сохранить фон: ' + e.message); }
    });
    el.appendChild(btn);
