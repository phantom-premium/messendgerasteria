...
const TABLES = ['users', 'sessions', 'conversations', 'messages', 'stories', 'folders', 'calls', 'bots', 'pushTokens', 'riveoProfiles', 'contacts'];
...
// Подписки Web Push (iOS/Android/десктоп-браузеры) хранятся в этой же таблице
// (endpoint играет роль token) — см. /api/push/subscribe в server.js.
db.exec(`CREATE INDEX IF NOT EXISTS idx_pushtokens_endpoint ON pushTokens(json_extract(data, '$.endpoint'))`);
db.exec(`CREATE INDEX IF NOT EXISTS idx_calls_callee ON calls(json_extract(data, '$.calleeId'))`);
// Контакты (см. /api/contacts в server.js) — статус "в контактах" у другого
// пользователя, отдельно от факта наличия переписки с ним (переписка/DM
// создаётся автоматически даже просто от открытия чужого профиля — см.
// ensureDMExists в GET /api/users/:id — поэтому это ОБЯЗАНА быть отдельная
// сущность, а не производная от списка бесед). id записи собран как
// `contact_<ownerId>_<contactUserId>` — гарантированно уникален сам по
// себе, отдельный композитный индекс не нужен, findById работает напрямую.
db.exec(`CREATE INDEX IF NOT EXISTS idx_contacts_owner ON contacts(json_extract(data, '$.ownerId'))`);
...
module.exports = { all, insert, update, remove, findById, findOneBy, findManyBy, findPageBy, findLastBy, countMessagesBy, removeManyBy, distinctFieldValues, incrementalVacuum, DB_PATH, TABLES, exec: (sql) => db.exec(sql) };
