...
const { genId, hashPassword, verifyPassword, parseCookies, serializeCookie, encryptSecret, decryptSecret } = require('./lib/util');
...
// ---------- Бот ----------
// bot_asteria — системный бот "Цифровой ID", цифровой ID пользователя внутри
// Asteria (раньше был просто ботом-помощником "Asteria Bot" — id намеренно
// не меняем, иначе у всех пользователей "потерялась" бы личная переписка
// с ним; меняем только видимые поля и добавляем запись в таблицу bots,
// чтобы у него появилось собственное мини-приложение).
...
function ensureBotUser() {
  ...
  } else if (existing.displayName !== 'Цифровой ID') {
    // Миграция для уже развёрнутых серверов, где этот бот ещё называется
    // по-старому ("asteria_bot"/"Asteria Bot") — id и вся история переписки
    // сохраняются, обновляется только бренд.
...
const RESERVED_USERNAMES = new Set(['asteria_bot', 'riveo', 'botcreator', 'admin', 'support', 'asteria']);
...
function handleWSMessage(user, conn, msg) {
        ...
        setTimeout(() => {
          ...
          if (otherId === BOT_ID) {
            if (trimmed === ADMIN_COMMAND) {
              ...
              replyText = botReply(user, message.content);
            }
  ...
  if (msg.type === 'forward') {
    ...
    targetIds.forEach((convId) => {
      ...
      broadcastToConversation(conv, { type: 'message', message: forwarded, notif: buildNotifPreview(conv, forwarded) }, null);
    });
    return;
  }
...
async function handleAPI(req, res, pathname, query = {}) {
  ...
  if (pathname === '/api/app/version' && method === 'GET') {
    ...
    if (!release) return sendJSON(res, 200, { available: false });
  ...
  if (pathname === '/api/app/download' && method === 'GET') {
    if (!fs.existsSync(APP_RELEASE_APK_PATH)) return sendJSON(res, 404, { error: 'Обновление ещё не загружено' });
    const release = readAppReleaseMeta();
    const stat = fs.statSync(APP_RELEASE_APK_PATH);
    res.writeHead(200, {
      'Content-Type': 'application/vnd.android.package-archive',
      'Content-Length': stat.size,
      'Content-Disposition': `attachment; filename="asteria-${release ? release.versionName : 'update'}.apk"`,
  ...
  if (convMsgMatch && method === 'GET') {
    ...
    return sendJSON(res, 200, { messages: items, hasMore });
  }
...
function enrichConversationAdmin(conv) {
  ...
  out.lastMessage = db.findLastBy('messages', 'conversationId', conv.id);
  return out;
}
