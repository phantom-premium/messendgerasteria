...
// ---------- Web Push (уведомления в закрытом Safari/PWA на iOS и в фоне других браузеров) ----------
// Ключи VAPID должны быть ПОСТОЯННЫМИ: браузер подписывается на push один раз
// с текущим публичным ключом сервера (applicationServerKey), и если ключ
// сменится, все существующие подписки станут недействительны и всем
// пользователям придётся заново включать уведомления. Поэтому храним пару
// в PERSIST_ROOT (переживает обновление кода/деплой), как и БД/загрузки, а
// не генерируем заново при каждом старте.
const VAPID_KEYS_PATH = path.join(PERSIST_ROOT, 'vapid-keys.json');
// "Контакт администратора" — обязательное поле VAPID (RFC 8292): по нему
// push-служба (Apple/Google/Mozilla) при необходимости может связаться с
// владельцем сервера (например, если он начнёт слать слишком много push).
// Переопределяется переменной окружения ASTERIA_PUSH_CONTACT.
// "Контакт администратора" — обязательное поле VAPID (RFC 8292): по нему
// push-служба (Apple/Google/Mozilla) при необходимости может связаться с
// владельцем сервера. ВАЖНО: Apple (web.push.apple.com) реально проверяет
// домен в mailto: и отклоняет "ненастоящие" домены вроде .local/.localhost
// с ошибкой 403 BadJwtToken (задокументированная особенность именно
// Apple — Google/Mozilla к этому терпимее). example.com — гарантированно
// проходит эту проверку. Перекрывается переменной окружения
// ASTERIA_PUSH_CONTACT (укажите свой настоящий email или https:-адрес).
const VAPID_SUBJECT = process.env.ASTERIA_PUSH_CONTACT || 'mailto:admin@example.com';
let vapidKeys = null;
try {
  ...
  console.log('🔑 Сгенерирована новая пара ключей VAPID для Web Push:', VAPID_KEYS_PATH);
}
...
function serveStatic(req, res, rootDir, urlPath) {
  ...
  fs.stat(filePath, (err, stat) => {
    ...
    fs.readFile(filePath, (err2, data) => {
      ...
      // sw.js обязан переспрашиваться при каждой загрузке — иначе браузер
      // (особенно Safari) может месяцами держать старую версию service
      // worker'а в кэше, и обновления логики push-уведомлений до устройств
      // просто не доходят.
...
// ФИКС: "звонок остаётся висеть на остальных устройствах, если ответить на
// одном". Раньше call-answer/call-decline пересылались только адресату
// (msg.to, т.е. другой стороне звонка) — но если у ОТВЕЧАЮЩЕГО мессенджер
// открыт сразу на нескольких устройствах (телефон + веб и т.п.), то
// call-offer уходит на ВСЕ его сокеты (см. sendToUser выше), и когда он
// принимает вызов на одном из них, остальные его устройства ничего об этом
// не узнают — экран входящего звонка так и остаётся висеть, будто вызов
// продолжается. Эта функция шлёт сиблинг-сокетам того же пользователя
// (кроме того, с которого пришло действие) отдельное уведомление, чтобы
// они могли закрыть у себя экран входящего звонка.
function notifyOtherDevices(userId, exceptConn, payload) {
  const set = socketsByUser.get(userId);
  if (!set) return;
  set.forEach((conn) => { if (conn !== exceptConn) conn.send(payload); });
}
...
// ---------- Web Push для новых сообщений и звонков ----------
// В отличие от notif-поля в WS-payload (которое видит только тот, у кого
// прямо сейчас открыто соединение — вкладка/WebView на экране или фоновый
// AsteriaPushService на Android), это будит браузер даже когда сайт нигде
// не открыт: Safari/iOS не даёт держать WS-соединение живым в закрытом
// PWA, поэтому единственный способ получить уведомление там — системный
// push через Service Worker (см. public/sw.js).
function pushTargetsForConversation(conv, exceptUserId) {
  const targets = (conv.type === 'channel' || conv.type === 'group') ? [...(conv.participants || []), conv.ownerId] : conv.participants;
  return [...new Set(targets)].filter((uid) => uid && uid !== exceptUserId);
}
// Общая отправка push одному пользователю (по всем его сохранённым
// подпискам сразу — может быть несколько устройств/браузеров). Вынесена
// из sendWebPushForMessage, чтобы тем же путём слать и уведомления о
// входящих звонках (см. sendWebPushForCall ниже) — логика доставки,
// очистки истёкших/невалидных подписок одна и та же для обоих случаев.
function sendWebPushToUser(uid, payload) {
  const subs = db.findManyBy('pushTokens', 'userId', uid);
  subs.forEach((sub) => {
    webpush.sendWebPush({ endpoint: sub.endpoint, keys: { p256dh: sub.p256dh, auth: sub.auth } }, payload, vapidKeys, VAPID_SUBJECT)
      .then((result) => {
        if (result.statusCode >= 200 && result.statusCode < 300) {
          console.log(`🔔 Web Push доставлен push-службе: пользователь ${uid}, статус ${result.statusCode}`);
        } else if (result.statusCode === 404 || result.statusCode === 410) {
          // 404/410 — подписка отозвана или устарела (переустановка, чистка
          // данных браузера и т.п.) — push-служба сама об этом сообщает,
          // дальше пытаться слать в неё бессмысленно, удаляем запись.
          console.log(`🔔 Подписка ${sub.id} более не действительна (${result.statusCode}) — удаляю`);
          db.remove('pushTokens', sub.id);
        } else {
          console.error(`⚠️ Web Push отклонён (${result.statusCode}) для пользователя ${uid}:`, result.body.slice(0, 300));
        }
      })
      .catch((e) => {
        console.error('⚠️ Ошибка отправки Web Push:', e.message);
        if (e && e.code === 'ERR_CRYPTO_ECDH_INVALID_PUBLIC_KEY') {
          // Эта конкретная подписка никогда не сможет получить push —
          // её ключ математически не является точкой на кривой P-256
          // (сохранена ещё до того, как эта проверка появилась на этапе
          // подписки). Удаляем, чтобы клиент при следующем открытии
          // приложения переподписался заново с валидным ключом.
          console.log(`🔔 Подписка ${sub.id} содержит невалидный ключ — удаляю, клиент переподпишется заново`);
          db.remove('pushTokens', sub.id);
        }
      });
  });
}
function sendWebPushForMessage(conv, message, notif) {
  const targets = pushTargetsForConversation(conv, message.senderId);
  targets.forEach((uid) => {
    sendWebPushToUser(uid, { title: notif.title, body: notif.body, tag: conv.id, conversationId: conv.id, url: '/' });
  });
}
// Push для входящего 1:1 звонка — по духу то же, что showCallNotification()
// в AsteriaPushService.java на Android (свой канал, высокий приоритет,
// открывает приложение, а экран входящего звонка дальше уже поднимает
// сама веб-страница через resendRingingCallsTo). tag = call-<callId> —
// на нём же строится "отбой" уведомления ниже (cancelWebPushForCall), если
// звонок закончился раньше, чем пользователь успел на него посмотреть.
function sendWebPushForCall(calleeId, callId, callerName, kind) {
  const body = kind === 'video' ? 'Входящий видеозвонок' : 'Входящий аудиозвонок';
  sendWebPushToUser(calleeId, { title: callerName || 'Входящий звонок', body, tag: `call-${callId}`, isCall: true, url: '/' });
}
// Присылает push, который не показывает новое уведомление, а лишь просит
// Service Worker закрыть уже показанное (по совпадающему tag) — нужно на
// случай, если звонящий положил трубку/дозвонился на другое устройство
// раньше, чем адресат вообще увидел системное уведомление о звонке (иначе
// оно так и осталось бы висеть в Центре уведомлений про уже неактуальный
// вызов — ровно та же причина, что и у cancelCallNotification() в Android).
function cancelWebPushForCall(calleeId, callId) {
  sendWebPushToUser(calleeId, { cancelCall: true, tag: `call-${callId}` });
}
...
function handleWSMessage(user, conn, msg) {
  ...
  if (msg.type === 'message') {
    ...
    const notifPreview = buildNotifPreview(conv, outMessage);
    broadcastToConversation(conv, { type: 'message', message: outMessage, notif: notifPreview }, null);
    sendWebPushForMessage(conv, outMessage, notifPreview);
    ...
    if (conv.type === 'dm' && user.id !== BOT_ID && user.id !== BOTCREATOR_ID) {
      ...
      if (otherUser && otherUser.isBot) {
        ...
        setTimeout(() => {
          ...
          const replyNotifPreview = buildNotifPreview(conv, outReply);
          broadcastToConversation(conv, { type: 'message', message: outReply, notif: replyNotifPreview }, null);
          sendWebPushForMessage(conv, outReply, replyNotifPreview);
        }, 500);
      }
    }
    return;
  ...
  if (msg.type === 'forward') {
    ...
    targetIds.forEach((convId) => {
      ...
      const forwardNotifPreview = buildNotifPreview(conv, outForwarded);
      broadcastToConversation(conv, { type: 'message', message: outForwarded, notif: forwardNotifPreview }, null);
      sendWebPushForMessage(conv, outForwarded, forwardNotifPreview);
    });
    return;
  }
  ...
  if (['call-offer', 'call-answer', 'call-ice', 'call-end', 'call-decline', 'call-media-toggle'].includes(msg.type)) {
    ...
    if (msg.type === 'call-offer' && msg.callId) {
      ...
      // ФИКС "звонок навсегда висит в checking/connecting": повторный
      // offer в рамках уже идущего звонка (ICE-restart, см. onnegotiationneeded
      // в public/app.js) — это НЕ новый входящий вызов. Раньше такой offer
      // тут обрабатывался бы неотличимо от первого: статус звонка откатывался
      // обратно на "ringing", и уходил повторный push/уведомление о входящем
      // звонке посреди уже идущего разговора (у собеседника снова звонил бы
      // телефон). Пропускаем запись в историю звонков и push для этого случая.
      if (!msg.isRenegotiation) {
        ...
        // Push для iOS/закрытых вкладок — см. комментарий у sendWebPushForCall
        // выше. sendToUser ниже по коду добьёт сигнал до уже открытых
        // соединений (в т.ч. фонового AsteriaPushService на Android), push —
        // отдельно, до тех, у кого сейчас вообще нет живого соединения.
        sendWebPushForCall(msg.to, msg.callId, user.displayName, msg.kind || 'audio');
      }
    } else if (msg.type === 'call-answer' && msg.callId && !msg.isRenegotiation) {
      ...
      // см. notifyOtherDevices выше: если ответили с одного устройства,
      // остальные устройства того же пользователя должны убрать у себя
      // экран входящего звонка.
      notifyOtherDevices(user.id, conn, { type: 'call-taken-elsewhere', callId: msg.callId });
      // Ответили на одном устройстве — если на других устройствах ЭТОГО ЖЕ
      // пользователя уже показался системный push-уведомление о звонке
      // (iOS), его нужно убрать, иначе останется висеть про уже принятый
      // звонок (см. cancelWebPushForCall выше).
      cancelWebPushForCall(user.id, msg.callId);
      ...
      // то же самое для отклонения: если отклонили на одном устройстве,
      // на остальных экран входящего звонка тоже нужно закрыть.
      notifyOtherDevices(user.id, conn, { type: 'call-taken-elsewhere', callId: msg.callId });
      cancelWebPushForCall(user.id, msg.callId);
      ...
      if (rec) {
        ...
        // Звонок закончился (положили трубку/не успели ответить) — убираем
        // возможно уже показанный push-уведомление у того, кому звонили
        // (rec.calleeId, а не msg.to — call-end может прислать и сам
        // звонящий, отменив вызов, и тот, кому звонили).
        cancelWebPushForCall(rec.calleeId, msg.callId);
      }
    }
...
async function handleAPI(req, res, pathname, query = {}) {
  ...
  // БАГ: ссылки на профиль и QR-коды строились из location.origin —
  // адреса, который браузер использовал ПРЯМО СЕЙЧАС, чтобы открыть
  // страницу. Если сервер настроен на нормальный домен (PUBLIC_HOST=
  // messenger.example.com, см. README — именно на него выписывается HTTPS-
  // сертификат), но конкретный человек в моменте зашёл по голому IP
  // (например, по старой ссылке, из истории браузера, или потому что так
  // было проще ввести вручную) — ссылка на его профиль/QR-код получалась
  // с тем же голым IP, хотя официальный, "сертифицированный" адрес сервиса
  // — это домен. Отдаём канонический публичный адрес отдельным эндпоинтом,
  // чтобы buildProfileLink() в app.js могла использовать именно его, а не
  // то, чем угодно оказался location.origin в конкретной вкладке.
  if (pathname === '/api/public-config' && method === 'GET') {
    const configuredHost = (process.env.PUBLIC_HOST || '').split(',')[0].trim() || null;
    return sendJSON(res, 200, { publicOrigin: configuredHost ? `https://${configuredHost}` : null });
  }
  ...
  // ---------- Web Push подписки ----------
  // Публичный ключ VAPID, которым клиент должен подписаться (applicationServerKey
  // в pushManager.subscribe) — см. subscribeToPush() в app.js.
  if (pathname === '/api/push/vapid-public-key' && method === 'GET') {
    return sendJSON(res, 200, { publicKey: vapidKeys.publicKey });
  }

  // Сохраняет подписку браузера/PWA на push. endpoint однозначно определяет
  // конкретную подписку конкретного браузера на конкретном устройстве —
  // если он уже был сохранён (переустановка, повторный вход и т.п.),
  // обновляем запись, а не плодим дубликаты.
  if (pathname === '/api/push/subscribe' && method === 'POST') {
    const body = await readBody(req);
    const endpoint = String((body && body.endpoint) || '').trim();
    const p256dh = body && body.keys && String(body.keys.p256dh || '').trim();
    const auth = body && body.keys && String(body.keys.auth || '').trim();
    if (!endpoint || !p256dh || !auth) return sendJSON(res, 400, { error: 'Некорректная push-подписка' });
    // Проверяем сразу на этом шаге, а не только в момент отправки push —
    // если ключ браузера не декодируется в корректную точку кривой P-256,
    // лучше явно отклонить подписку с понятной причиной в логе, чем молча
    // сохранить нерабочие данные и упасть намного позже при реальной
    // отправке (см. ERR_CRYPTO_ECDH_INVALID_PUBLIC_KEY).
    let p256dhBuf, authBuf;
    try {
      p256dhBuf = webpush.fromB64url(p256dh);
      authBuf = webpush.fromB64url(auth);
    } catch (e) {
      console.error(`⚠️ Push-подписка отклонена: не удалось декодировать base64 (${e.message})`);
      return sendJSON(res, 400, { error: 'Некорректный формат ключей подписки' });
    }
    if (p256dhBuf.length !== 65 || p256dhBuf[0] !== 0x04) {
      console.error(`⚠️ Push-подписка отклонена: p256dh имеет длину ${p256dhBuf.length} байт (ожидалось 65, первый байт 0x04), первые байты: ${p256dhBuf.slice(0, 8).toString('hex')}, исходная строка (${p256dh.length} симв.): ${p256dh}`);
      return sendJSON(res, 400, { error: 'Некорректный публичный ключ подписки (p256dh)' });
    }
    // Проверка длины/префикса выше — только поверхностная. Настоящая
    // проверка "лежит ли эта точка на самой кривой P-256" — единственная,
    // которая по-настоящему предсказывает ERR_CRYPTO_ECDH_INVALID_PUBLIC_KEY
    // при реальной отправке push — это попытка computeSecret прямо сейчас,
    // с одноразовым ключом сервера (результат никуда не идёт, только
    // проверяем, что вызов не бросает исключение).
    try {
      const probe = crypto.createECDH('prime256v1');
      probe.generateKeys();
      probe.computeSecret(p256dhBuf);
    } catch (e) {
      console.error(`⚠️ Push-подписка отклонена: p256dh не является точкой на кривой P-256 (${e.code || e.message}). Полный base64url (${p256dh.length} симв.): ${p256dh} — hex (${p256dhBuf.length} байт): ${p256dhBuf.toString('hex')}`);
      return sendJSON(res, 400, { error: 'Ключ подписки не проходит проверку кривой P-256' });
    }
    if (authBuf.length !== 16) {
      console.error(`⚠️ Push-подписка отклонена: auth имеет длину ${authBuf.length} байт (ожидалось 16), исходная строка (${auth.length} симв.): ${auth}`);
      return sendJSON(res, 400, { error: 'Некорректный auth-секрет подписки' });
    }
    const existing = db.findOneBy('pushTokens', 'endpoint', endpoint);
    if (existing) {
      db.update('pushTokens', existing.id, { userId: user.id, p256dh, auth, userAgent: String(req.headers['user-agent'] || '').slice(0, 300), updatedAt: Date.now() });
    } else {
    ...
    console.log(`🔔 Push-подписка сохранена: пользователь ${user.id}, endpoint ${endpoint.slice(0, 60)}…`);
    return sendJSON(res, 200, { ok: true });
  }

  // Отписка (пользователь выключил уведомления в настройках, либо клиент
  // сам обнаружил, что подписка истекла — pushManager.subscribe отдаёт
  // новый endpoint при ротации, старый нужно явно удалить).
  if (pathname === '/api/push/unsubscribe' && method === 'POST') {
    const body = await readBody(req);
    const endpoint = String((body && body.endpoint) || '').trim();
    if (!endpoint) return sendJSON(res, 400, { error: 'Не указан endpoint' });
    const existing = db.findOneBy('pushTokens', 'endpoint', endpoint);
    if (existing && existing.userId === user.id) db.remove('pushTokens', existing.id);
    return sendJSON(res, 200, { ok: true });
  }
  ...
  if (pathname === '/api/me' && method === 'PATCH') {
    ...
    // Рингтон звонка: "classic" — встроенный звук (public/ringtones/classic.mp3),
    // "custom" — свой загруженный файл (см. /api/upload с kind: 'ringtone'
    // ниже). Здесь только переключаем выбор и/или сохраняем ссылку на уже
    // загруженный файл — сама загрузка отдельным запросом.
    if (body.ringtoneType !== undefined) {
      if (!['classic', 'custom'].includes(body.ringtoneType)) return sendJSON(res, 400, { error: 'Неизвестный тип рингтона' });
      patch.ringtoneType = body.ringtoneType;
    }
    if (body.ringtoneUrl !== undefined) patch.ringtoneUrl = body.ringtoneUrl;
  ...
  if (userProfileMatch && method === 'GET') {
    ...
    if (out && target.id !== user.id) {
      ...
      // Статус "в контактах" — см. комментарий у contacts-таблицы в db.js.
      // Тот же приоритет источников имени, что и в enrichConversation ниже
      // (список чатов): сначала новая таблица contacts, а если там пусто —
      // старое имя, заданное ещё при создании чата через поиск (compose-
      // флоу, conversations.nicknames). ФИКС: раньше тут проверялась
      // только contacts, и имя, заданное старым способом, не показывалось
      // в самом профиле — хотя в списке чатов уже было видно.
      const contact = db.findById('contacts', `contact_${user.id}_${target.id}`);
      const legacyNickname = conv.nicknames && conv.nicknames[user.id];
      out.contactNickname = (contact && contact.nickname) || legacyNickname || '';
      // Если имя когда-либо было задано (пусть даже только старым способом,
      // без явного "Добавить в контакты") — по смыслу это уже контакт:
      // иначе получилось бы странно — сверху написано заданное имя, а
      // снизу всё равно кнопка "Добавить в контакты" вместо "Изменить".
      out.isContact = !!contact || !!legacyNickname;
    }
    return sendJSON(res, 200, { user: out });
  }

  // ---------- Контакты ----------
  // Добавить человека в контакты (кнопка "Добавить в контакты" в профиле —
  // см. openProfilePage в app.js). Идемпотентно: повторный вызов на уже
  // добавленного контакта просто обновит nickname, если он передан, не
  // создавая дубликатов (id составной, см. комментарий в lib/db.js).
  if (pathname === '/api/contacts' && method === 'POST') {
    const body = await readBody(req);
    const targetId = String((body && body.userId) || '').trim();
    if (!targetId || targetId === user.id) return sendJSON(res, 400, { error: 'Некорректный пользователь' });
    const target = db.findById('users', targetId);
    if (!target) return sendJSON(res, 404, { error: 'Пользователь не найден' });
    const nickname = String((body && body.nickname) || '').trim().slice(0, 60);
    const id = `contact_${user.id}_${targetId}`;
    const existing = db.findById('contacts', id);
    const record = existing
      ? db.update('contacts', id, { nickname: nickname || existing.nickname || '' })
      : db.insert('contacts', { id, ownerId: user.id, contactUserId: targetId, nickname, createdAt: Date.now() });
    return sendJSON(res, 200, { contact: record });
  }

  // Изменить локальное имя контакта (кнопка "Изменить" в правом верхнем
  // углу профиля — показывается, если человек уже в контактах, в том числе
  // "по факту" — см. isContact выше, куда попадает и старое legacy-имя).
  // Ведёт себя как upsert: формальной записи в contacts могло ещё не быть
  // (например, имя раньше было задано только старым способом при создании
  // чата) — в этом случае создаём её прямо сейчас, а не требуем, чтобы она
  // существовала заранее.
  const contactPatchMatch = pathname.match(/^\/api\/contacts\/([^/]+)$/);
  if (contactPatchMatch && method === 'PATCH') {
    const targetId = contactPatchMatch[1];
    if (targetId === user.id) return sendJSON(res, 400, { error: 'Некорректный пользователь' });
    if (!db.findById('users', targetId)) return sendJSON(res, 404, { error: 'Пользователь не найден' });
    const id = `contact_${user.id}_${targetId}`;
    const existing = db.findById('contacts', id);
    const body = await readBody(req);
    const nickname = String((body && body.nickname) || '').trim().slice(0, 60);
    const record = existing
      ? db.update('contacts', id, { nickname })
      : db.insert('contacts', { id, ownerId: user.id, contactUserId: targetId, nickname, createdAt: Date.now() });
    return sendJSON(res, 200, { contact: record });
  }

  // Убрать из контактов (доступно из того же диалога изменения имени).
  if (contactPatchMatch && method === 'DELETE') {
    const id = `contact_${user.id}_${contactPatchMatch[1]}`;
  ...
  if (pathname === '/api/upload' && method === 'POST') {
    ...
    const subdir = kind === 'avatar' ? 'avatars' : kind === 'story' ? 'stories' : kind === 'wallpaper' ? 'wallpapers' : kind === 'ringtone' ? 'ringtones' : '';
  ...
  if (msgItemMatch && method === 'PATCH') {
    ...
    // БАГ: пересланное сообщение хранится с senderId = того, кто переслал
    // (см. msg.type === 'forward' выше — forwardFrom описывает исходного
    // автора отдельным полем именно для этого), поэтому проверка выше
    // ("это моё сообщение") проходила и для пересланных сообщений тоже.
    // Из-за этого можно было переслать чужое сообщение себе/в другой чат
    // и затем отредактировать текст — получатели видели бы поддельное
    // содержимое под меткой "Переслано от <автор>", как будто это
    // реальные слова исходного отправителя. Содержимое пересылки должно
    // быть неизменяемым слепком оригинала.
    if (message.forwardFrom && !user.isAdmin) return sendJSON(res, 403, { error: 'Пересланные сообщения нельзя редактировать' });
    const body = await readBody(req);
    ...
    // БАГ-ФИКС: тут раньше рассылался "сырой" updated-объект из БД поверх
    // подмены только content — а вложенный replyPreview.content (снимок
    // текста цитируемого сообщения) в БД хранится зашифрованным и никогда
    // не расшифровывался на этом пути. В итоге у собеседников, уже открывших
    // чат, после любого редактирования ЛЮБОГО сообщения цитата в ответах на
    // это сообщение начинала показывать шифротекст вместо текста. Проходим
    // updated через тот же decryptMessage(), что и обычное чтение истории
    // (расшифрует replyPreview.content), и уже поверх подставляем свежий
    // newContent — сам updated.content расшифровывать не нужно, он и так
    // известен нам в открытом виде.
    const outUpdated = { ...decryptMessage(updated), content: newContent };
    const conv = db.findById('conversations', message.conversationId);
  ...
  // Два формата на выбор в 5-й вкладке админ-панели:
  //   1) настоящий файл SQLite (.db) — самый надёжный вариант бэкапа:
  //      просто кладёте его обратно вместо ~/.asteria-data/db/asteria.db
  //      при восстановлении, ничего не нужно пересобирать/парсить заново.
  //   2) человекочитаемый JSON — удобно, если нужно just посмотреть данные
  //      глазами или перенести их во что-то другое, не обязательно в SQLite.
  if (pathname === '/api/admin/export/db' && method === 'GET') {
    if (!requireAdmin(user, res)) return;
    // WAL-режим означает, что часть самых свежих записей может физически
    // лежать ещё не в основном файле .db, а в соседнем файле -wal — если
    // отдать на скачивание только сам .db, не сделав чекпоинт, в выгрузке
    // могли бы не оказаться последние сообщения/действия. TRUNCATE-чекпоинт
    // сбрасывает всё содержимое WAL в основной файл и опустошает сам WAL —
    // после него отдаваемый .db гарантированно самодостаточен и актуален.
    db.exec('PRAGMA wal_checkpoint(TRUNCATE);');
    if (!fs.existsSync(db.DB_PATH)) return sendJSON(res, 500, { error: 'Файл базы данных не найден' });
    const stat = fs.statSync(db.DB_PATH);
    const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    res.writeHead(200, {
      'Content-Type': 'application/octet-stream',
      'Content-Length': stat.size,
      'Content-Disposition': `attachment; filename="asteria-backup-${stamp}.db"`,
      'Cache-Control': 'no-store',
    });
    fs.createReadStream(db.DB_PATH).pipe(res);
    return;
  }
  if (pathname === '/api/admin/export/json' && method === 'GET') {
    if (!requireAdmin(user, res)) return;
    const dump = { exportedAt: new Date().toISOString(), tables: {} };
    db.TABLES.forEach((t) => { dump.tables[t] = db.all(t); });
    const json = JSON.stringify(dump, null, 2);
    const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    res.writeHead(200, {
      'Content-Type': 'application/json; charset=utf-8',
      'Content-Length': Buffer.byteLength(json),
      'Content-Disposition': `attachment; filename="asteria-export-${stamp}.json"`,
      'Cache-Control': 'no-store',
    });
    res.end(json);
    return;
  ...
  // ---------- Полный архив для переезда на другой сервер ----------
  // В отличие от .db/.json выше, сюда же кладём и все загруженные файлы
  // (аватарки, обои, сторис) — без них одной базы недостаточно, чтобы
  // картинки не превратились в "битые" ссылки на новом сервере.
  //
  // Структура внутри архива НАРОЧНО повторяет структуру постоянного
  // каталога данных (".asteria-data/db/asteria.db", ".asteria-data/uploads/...")
  // — см. PERSIST_ROOT в lib/db.js и здесь же выше. Благодаря этому на
  // новом сервере ничего руками раскладывать по подпапкам не нужно:
  // распаковать архив прямо в домашнюю директорию пользователя, от
  // которого будет запущен сервер (по умолчанию именно там сервер и ищет
  // данные — ~/.asteria-data), и всё встанет на свои места само.
  if (pathname === '/api/admin/export/archive' && method === 'GET') {
    if (!requireAdmin(user, res)) return;
    db.exec('PRAGMA wal_checkpoint(TRUNCATE);'); // см. пояснение у /api/admin/export/db выше
    const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    res.writeHead(200, {
      'Content-Type': 'application/zip',
      'Content-Disposition': `attachment; filename="asteria-full-backup-${stamp}.zip"`,
      'Cache-Control': 'no-store',
    });
    ...
    try {
      if (fs.existsSync(db.DB_PATH)) {
        await zip.addFile('.asteria-data/db/asteria.db', db.DB_PATH);
      }
      // Человекочитаемый JSON той же базы — не обязателен для восстановления
      // (для этого достаточно .db) и НЕ является частью реальной структуры
      // ~/.asteria-data, поэтому лежит рядом отдельным файлом, а не внутри
      // db/, чтобы случайно ничего там не перепутать при распаковке.
      const dump = { exportedAt: new Date().toISOString(), tables: {} };
      db.TABLES.forEach((t) => { dump.tables[t] = db.all(t); });
      await zip.addFile('.asteria-data/_readable-export.json', Buffer.from(JSON.stringify(dump, null, 2), 'utf8'));

      async function addDirToZip(dir, zipPrefix) {
        let entries;
        try { entries = fs.readdirSync(dir, { withFileTypes: true }); } catch (e) { return; }
        for (const entry of entries) {
          if (zip.aborted) return;
          const fullPath = path.join(dir, entry.name);
          if (entry.isDirectory()) { await addDirToZip(fullPath, zipPrefix + entry.name + '/'); continue; }
          await zip.addFile('.asteria-data/uploads/' + zipPrefix + entry.name, fullPath);
        }
      }
      await addDirToZip(UPLOADS_DIR, '');

      await zip.finalize();
    } catch (e) {
      console.error('⚠️  Ошибка при формировании полного архива экспорта:', e.message);
    } finally {
      res.end();
    }
    return;
  ...
  if (pathname === '/api/turn-credentials' && method === 'GET') {
    ...
    const iceServers = [
      ...
      {
        // Оба варианта транспорта клиент-сервер (UDP и TCP) — браузер сам
        // попробует оба и использует тот, что реально прошёл через сеть.
        // См. подробный комментарий в lib/mini-turn.js про TCP-фолбэк —
        // без него звонки могли не устанавливаться в сетях, блокирующих
        // исходящий UDP на нестандартные порты (частая причина того, что
        // звонок работает в одном Wi-Fi, но не работает между разными
        // сетями/через мобильный интернет).
...
function enrichConversation(conv, viewerId) {
  ...
  if (conv.type === 'dm') {
    ...
    // Приоритет: имя из "Контактов" (задаётся кнопкой "Изменить" в профиле)
    // важнее старого имени, заданного при создании чата через поиск — так
    // человек, осознанно добавивший контакт и переименовавший его позже,
    // видит именно актуальное имя, а не то, что ввёл мимоходом когда-то
    // при первом сообщении.
    const contactRec = db.findById('contacts', `contact_${viewerId}_${otherId}`);
    out.peerNickname = (contactRec && contactRec.nickname) || (conv.nicknames && conv.nicknames[viewerId]) || '';
...
// Диапазон UDP-портов для РЕЛЕЯ медиапотока (не путать с TURN_PORT — тот
// только для служебных STUN/TURN-сообщений). Без него каждый звонок брал
// случайный порт от ОС, который невозможно заранее пробросить через
// файрвол/облако — из-за этого Allocate проходил успешно, а сам медиапоток
// снаружи не доходил, и звонок зависал на "checking"/"connecting". По
// умолчанию — небольшой фиксированный диапазон (100 портов, хватает на
// полсотни одновременных релей-соединений), который достаточно один раз
// пробросить наружу так же, как TURN_PORT. Можно переопределить
// переменными окружения TURN_RELAY_MIN_PORT / TURN_RELAY_MAX_PORT.
const TURN_RELAY_MIN_PORT = Number(process.env.TURN_RELAY_MIN_PORT) || 49160;
const TURN_RELAY_MAX_PORT = Number(process.env.TURN_RELAY_MAX_PORT) || 49260;
...
async function startTurnServer() {
  ...
  try {
    await createTurnServer({
      secret: TURN_SECRET,
      port: TURN_PORT,
      publicIp,
      relayPortMin: TURN_RELAY_MIN_PORT,
      relayPortMax: TURN_RELAY_MAX_PORT,
      log: (m) => console.error('TURN:', m),
    });
    console.log(`🧊 TURN/STUN сервер запущен на порту ${TURN_PORT} (адрес для клиентов: ${TURN_HOST})`);

    // Лучший эффорт: сами пытаемся открыть нужные порты в файрволе ОС
    // (ufw/firewalld/iptables), чтобы звонки работали сразу, без ручного
    // "проброса портов" — самая частая причина, почему на свежем VPS всё
    // остальное работает, а звонки зависают. Если сервер работает без прав
    // root, или порты блокируются "снаружи" ОС (роутер дома / облачный
    // firewall провайдера) — это НЕ решить кодом внутри сервера, ниже будет
    // явное предупреждение об этом.
    const { autoOpenPorts } = require('./lib/firewall');
    const portRules = [
      { port: PORT, proto: 'tcp' },
      { port: HTTPS_PORT, proto: 'tcp' },
      { port: TURN_PORT, proto: 'udp' },
      { port: TURN_PORT, proto: 'tcp' },
      { portStart: TURN_RELAY_MIN_PORT, portEnd: TURN_RELAY_MAX_PORT, proto: 'udp' },
    ];
    const fw = autoOpenPorts(portRules, (m) => console.log('🔥 Файрвол:', m));
    if (fw.attempted) {
      console.log(`🔥 Файрвол: порты открыты автоматически через ${fw.tool}.`);
    } else {
      console.log('🔥 Файрвол: не удалось настроить автоматически (нет root или подходящего инструмента).');
      console.log(`   Откройте вручную (в панели хостинга или на сервере): ${PORT}/TCP, ${HTTPS_PORT}/TCP,`);
      console.log(`   ${TURN_PORT}/UDP+TCP, ${TURN_RELAY_MIN_PORT}-${TURN_RELAY_MAX_PORT}/UDP.`);
    }
    console.log('   Если сервер стоит дома за обычным Wi-Fi-роутером ИЛИ хостинг-провайдер держит');
    console.log('   отдельный "облачный" файрвол (AWS Security Group, DO/Timeweb Cloud Firewall и');
    console.log('   т.п.) — это отдельная настройка вне этого сервера, автоматически её не открыть,');
    console.log('   нужно один раз включить эти же порты в панели роутера/хостинга (см. README).');
  } catch (e) {
