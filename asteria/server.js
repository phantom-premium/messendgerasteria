...
async function handleAPI(req, res, pathname, query = {}) {
  ...
  if (pathname === '/api/register' && method === 'POST') {
    ...
    // авто-DM с ботом и с BotCreator (создание собственных ботов)
    ensureDMExists(user.id, BOT_ID);
    ensureDMExists(user.id, BOTCREATOR_ID);
  ...
  if (pathname === '/api/bots' && method === 'GET') {
    const allBots = db.all('bots').filter((b) => b.ownerId && !b.isSystem);
  ...
  if (botMatch && method === 'PATCH') {
    ...
    if (body.miniApp !== undefined) {
      ...
      const miniUrl = String((body.miniApp && body.miniApp.url) || '').trim().slice(0, 500);
      if (enabled && miniUrl && !/^https?:\/\//i.test(miniUrl)) {
        return sendJSON(res, 400, { error: 'Ссылка мини-приложения должна начинаться с http:// или https://' });
      }
