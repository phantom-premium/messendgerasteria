...
// Токен для бесплатного (rate-limited) Hugging Face Inference API — используется
// сервером для расшифровки голосовых/видео-кружков в текст через модель Whisper.
// Получить свой токен можно бесплатно на https://huggingface.co/settings/tokens
// и передать при запуске: HF_API_TOKEN=hf_xxx node server.js
// Без токена эндпоинт /api/transcribe вернёт понятную ошибку, остальной
// функционал мессенджера при этом продолжает работать как обычно.
...
const RESERVED_USERNAMES = new Set(['asteria_bot', 'botcreator', 'admin', 'support', 'asteria']);
...
function publicUserForViewer(u, viewerId) {
  ...
  out.online = isSelf ? socketsByUser.has(u.id) : (socketsByUser.has(u.id) && !u.hideOnlineStatus);
...
const PREMIUM_MAX_UPLOAD_BYTES = 120 * 1024 * 1024; // 120 МБ
const REGULAR_MAX_UPLOAD_BYTES = 25 * 1024 * 1024;  // 25 МБ
...
function handleWSMessage(user, conn, msg) {
  ...
  if (msg.type === 'message') {
    ...
    broadcastToConversation(conv, { type: 'message', message, notif: buildNotifPreview(conv, message) }, null);
      ...
      if (otherUser && otherUser.isBot) {
        ...
        setTimeout(() => {
          ...
          if (otherId === BOT_ID) {
            ...
            replyText = botCreatorReply(user, message.content);
          } else {
            const botRecord = db.findById('bots', otherId);
            replyText = botRecord ? customBotReply(botRecord, message.content) : 'Этот бот больше не существует.';
          }
          ...
          broadcastToConversation(conv, { type: 'message', message: reply, notif: buildNotifPreview(conv, reply) }, null);
        }, 500);
      }
    }
    return;
...
async function handleAPI(req, res, pathname, query = {}) {
  ...
  if (userProfileMatch && method === 'GET') {
    ...
    return sendJSON(res, 200, { user: publicUserForViewer(target, user.id) });
  }
  ...
  // Расшифровка голосового/видео-кружка в текст через бесплатный внешний
  // Hugging Face Inference API (модель Whisper). Токен и сам HTTP-запрос
  // к внешнему сервису живут только на сервере — клиент его не видит,
  // просто присылает mediaUrl уже загруженного файла и получает текст.
  if (pathname === '/api/transcribe' && method === 'POST') {
    if (!HF_API_TOKEN) {
      return sendJSON(res, 501, {
        error: 'Расшифровка не настроена на сервере: нет HF_API_TOKEN. Получите бесплатный токен на huggingface.co/settings/tokens и запустите сервер с HF_API_TOKEN=hf_xxx node server.js',
      });
    }
    const body = await readBody(req);
    const mediaUrl = String(body.mediaUrl || '');
    if (!mediaUrl.startsWith('/uploads/')) return sendJSON(res, 400, { error: 'Некорректный файл' });
    const relPath = mediaUrl.slice('/uploads/'.length);
    const resolvedPath = path.resolve(path.join(UPLOADS_DIR, relPath));
    const resolvedRoot = path.resolve(UPLOADS_DIR) + path.sep;
    if (!resolvedPath.startsWith(resolvedRoot)) return sendJSON(res, 400, { error: 'Некорректный путь' });
    if (!fs.existsSync(resolvedPath)) return sendJSON(res, 404, { error: 'Файл не найден' });

    let fileBuffer;
    try {
      fileBuffer = fs.readFileSync(resolvedPath);
    } catch (e) {
      return sendJSON(res, 500, { error: 'Не удалось прочитать файл' });
    }

    const ext = path.extname(resolvedPath).toLowerCase();
    const contentType = { '.webm': 'audio/webm', '.mp3': 'audio/mpeg', '.wav': 'audio/wav', '.ogg': 'audio/ogg', '.m4a': 'audio/mp4', '.mp4': 'video/mp4' }[ext] || 'application/octet-stream';

    try {
      const hfResp = await fetch(`https://api-inference.huggingface.co/models/${encodeURIComponent(HF_STT_MODEL)}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${HF_API_TOKEN}`, 'Content-Type': contentType },
        body: fileBuffer,
      });
      const data = await hfResp.json().catch(() => ({}));
      if (!hfResp.ok) {
        if (data && data.estimated_time) {
          return sendJSON(res, 503, { error: `Модель распознавания ещё "прогревается" на сервере Hugging Face — попробуйте ещё раз через ~${Math.ceil(data.estimated_time)} сек.` });
        }
        return sendJSON(res, 502, { error: (data && data.error) || 'Сервис распознавания речи вернул ошибку' });
      }
      const text = (data && data.text) || (Array.isArray(data) && data[0] && data[0].text) || '';
      return sendJSON(res, 200, { text: String(text).trim() });
    } catch (e) {
      return sendJSON(res, 502, { error: 'Не удалось связаться с сервисом распознавания речи' });
    }
  }
...
function enrichConversation(conv, viewerId) {
  ...
  // (для бейджа в списке чатов слева).
  out.reads = visibleReadsFor(conv, viewerId);
  const myLastRead = (conv.reads && conv.reads[viewerId]) || 0;
  out.unreadCount = msgs.filter((m) => m.senderId !== viewerId && m.createdAt > myLastRead).length;
