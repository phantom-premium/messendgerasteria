...
async function handleAPI(req, res, pathname, query = {}) {
  ...
  // ---------- Диагностика звонков ----------
  // Раньше единственным способом понять, ПОЧЕМУ конкретный звонок не
  // соединился (или соединился, но без звука/видео), было смотреть живой
  // поток [CALL-DIAG] в терминале сервера ровно в момент звонка — если не
  // смотрел в эту минуту, данные пропадали безвозвратно. Теперь клиент
  // (см. sendCallDiagnostics в app.js) в конце каждого звонка сам присылает
  // сюда итоговую структурированную сводку — какие типы ICE-кандидатов
  // собрались, какая пара была выбрана (напрямую/через STUN/через TURN,
  // и по какому протоколу — UDP или TCP), финальное состояние соединения,
  // видеокарта/браузер — и она сохраняется вместе с записью о звонке,
  // доступна потом через историю звонков в любой момент.
  const callDiagMatch = pathname.match(/^\/api\/calls\/([^/]+)\/diagnostics$/);
  if (callDiagMatch && method === 'POST') {
    const call = db.findById('calls', callDiagMatch[1]);
    if (!call) return sendJSON(res, 404, { error: 'Звонок не найден' });
    const isParticipant = call.mode === '1:1'
      ? (call.callerId === user.id || call.calleeId === user.id)
      : userConversations(user.id).some((c) => c.id === call.conversationId);
    if (!isParticipant) return sendJSON(res, 403, { error: 'Это не ваш звонок' });
    const body = await readBody(req);
    const role = body && body.role === 'callee' ? 'callee' : 'caller';
    let summary = body && body.summary && typeof body.summary === 'object' ? body.summary : {};
    // Ограничиваем размер на случай испорченного/огромного пейлоада с
    // клиента — это диагностика, а не критичные данные, раздувать базу
    // смысла нет. Режем по количеству записей в самом объёмном поле
    // (events), а не "вслепую" по строке JSON — иначе можно случайно
    // обрезать посреди структуры и получить невалидный JSON.
    if (Array.isArray(summary.events) && summary.events.length > 200) {
      summary = { ...summary, events: summary.events.slice(-200) };
    }
    const diagnostics = { ...(call.diagnostics || {}), [role]: { ...summary, receivedAt: Date.now() } };
    db.update('calls', call.id, { diagnostics });
    return sendJSON(res, 200, { ok: true });
  }
