...
final class AppUpdateManager {
    ...
    // Раньше НИЧЕГО, кроме "Пропустить эту версию", не запоминалось — ни
    // нажатие "Позже", ни закрытие диалога свайпом/назад (он же
    // setCancelable(true)). Из-за этого при каждом новом запуске приложения
    // (checkForUpdate вызывается из onPageFinished — то есть буквально при
    // каждой загрузке страницы) диалог "Доступно обновление" выскакивал
    // заново и заново, пока пользователь не нажимал именно "Пропустить" —
    // даже если он уже согласился и установил обновление, но не убил
    // процесс приложения целиком (BuildConfig.VERSION_CODE — это константа,
    // вкомпилированная в уже ЗАПУЩЕННЫЙ процесс; она обновится только после
    // полного перезапуска приложения, а не сразу после установки APK поверх
    // работающего). Теперь запоминаем время последнего показа диалога для
    // конкретной версии и не показываем его повторно раньше, чем через
    // UPDATE_REMIND_COOLDOWN_MS — независимо от того, каким способом диалог
    // был закрыт (кнопкой, свайпом, назад).
    private static final String KEY_LAST_PROMPTED_VERSION = "update_last_prompted_version_code";
    private static final String KEY_LAST_PROMPTED_AT = "update_last_prompted_at";
    private static final long UPDATE_REMIND_COOLDOWN_MS = TimeUnit.HOURS.toMillis(20);
        ...
        buildHttpClient().newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) {
                try (ResponseBody body = response.body()) {
                    ...
                    // Эту же версию уже предлагали недавно (см. комментарий
                    // у полей выше) — не показываем диалог повторно на
                    // каждый чих, даём пользователю спокойно попользоваться
                    // приложением и напомним позже.
                    boolean samePromptedVersion = prefs.getInt(KEY_LAST_PROMPTED_VERSION, 0) == serverVersionCode;
                    long lastPromptedAt = prefs.getLong(KEY_LAST_PROMPTED_AT, 0);
                    if (samePromptedVersion && System.currentTimeMillis() - lastPromptedAt < UPDATE_REMIND_COOLDOWN_MS) return;
                    prefs.edit()
                            .putInt(KEY_LAST_PROMPTED_VERSION, serverVersionCode)
